...
getMonthForValueId = function(valueId){
    for (var property in months) {
        if (months.hasOwnProperty(property) && months[property] == valueId) {
            return property;
        }
    }
};
...
/**
 * Gets a list of all parent categories
 * @param type
 */
getAllParentCategories = function (type) {
    return BudgetCategory.find({type: type, owner: Meteor.userId(), parent_category: ""},
        {sort: {sorting_name: 1}});
};
...
/**
 * Gets all the chidren categories of a parent Category
 * @param parentCategory
 */
getAllChildOfParent = function(parentCategory){
    var filter = {
        $and: [
            {owner: Meteor.userId()}
        ]
    ...
    if (parentCategory) {
        filter['parent_category'] = parentCategory;
    }
    return BudgetCategory.find(
        filter,
        {sort: {sorting_name: 1}});
...
getCategory = function (categoryId) {
    return BudgetCategory.findOne({_id: categoryId});
};
...
/**
 * GEts the list of transaction for an account
 * @param accountId
 * @returns {*}
 */
getTransactionsOfAccount = function (accountId) {
    return Transaction.find({account_id: accountId}, {sort: {date: -1}});
};
